...
module GameState =
    ...
    let getPosition ({ Position = (CellPosition position) }) = position

    let getVisualPosition cell = getPosition cell + 1
    ...
    let getListOfValues (cells: Cell list) : string list = cells |> List.map getValue
